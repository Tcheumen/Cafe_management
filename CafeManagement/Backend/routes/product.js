...
router.post('/add', auth.authenficateToken, checkRole.checkRole, (req, res) => {
...
router.get('/get', auth.authenficateToken, (req, res, next) => {
...
router.get('/getByCategory/:id', auth.authenficateToken, (req, res, next) => {
...
router.get('/getById/:id', auth.authenficateToken, (req, res, next) => {
...
router.patch('/update', auth.authenficateToken, checkRole.checkRole, (req, res, next) => {
    let product = req.body;
...
router.delete('/delete/:id', auth.authenficateToken, checkRole.checkRole, (req, res, next) => {
    const id = req.params.id;
    ...
    connection.query(query, [id], (err, results) => {
        if (!err) {
            if (results.affectedRows == 0) {
               ...
               return res.status(200).json({ message: "Product Deleded Successfuly" });
            }
...
router.patch('/updatestatus', auth.authenficateToken, checkRole.checkRole, (req, res, next) => {
    let user = req.body;
    ...
    connection.query(query, [user.status, user.id], (err, results) => {
        if (!err) {
            ...
            return res.status(200).json({ message: "Product status Updeate Successfully" });
        } else {
            return res.status(500).json(err);
        }
    })
